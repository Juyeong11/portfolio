...
public:
	...
	void SetUavGpuDescriptorHandle(int nIndex, D3D12_GPU_DESCRIPTOR_HANDLE d3dSrvGpuDescriptorHandle);
...
{
public:
	CRotatingObject(float fWidth, float fHeight, float fLength, int nMesh = 1);
...
public:
	...
	float GetHeight(float x, float z, bool bReverseQuad = false) { return(m_pHeightMapImage->GetHeight(x, z, cxQuadsPerBlock, czQuadsPerBlock) * m_xmf3Scale.y); } //World
...
{
public:
	CExplosionObject(float fWidth, float fHeight, float fLength, int nMesh = 1);
...
public:
	...
	void BuildObject(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList);
	virtual void UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList);
...
public:
	...
	virtual void Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera = NULL);
	void SetStartPosition(const XMFLOAT3& xmf3StartPositon);
	void SetShootDirection(const XMFLOAT3& xmf3Look, const XMFLOAT3& xmf3Right, const XMFLOAT3& xmf3Up);
