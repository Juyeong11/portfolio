...
class CNormalTexturedVertex : public CIlluminatedTexturedVertex
{
public:
	XMFLOAT3						m_xmf3TangentU;

public:
	CNormalTexturedVertex() { m_xmf3Position = XMFLOAT3(0.0f, 0.0f, 0.0f); m_xmf2TexCoord = XMFLOAT2(0.0f, 0.0f); m_xmf3Normal = XMFLOAT3(0.0f, 0.0f, 0.0f); m_xmf3TangentU = XMFLOAT3(0.0f, 0.0f, 0.0f);}
	//CNormalTexturedVertex(float x, float y, float z, XMFLOAT2 xmf2TexCoord, XMFLOAT3 xmf3Normal = XMFLOAT3(0.0f, 0.0f, 0.0f)) { m_xmf3Position = XMFLOAT3(x, y, z); m_xmf3Normal = xmf3Normal; m_xmf2TexCoord = xmf2TexCoord; }
	CNormalTexturedVertex(XMFLOAT3 xmf3Position, XMFLOAT3 xmf3Normal = XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT2 xmf2TexCoord = XMFLOAT2(0.0f, 0.0f), XMFLOAT3 xmf3Tangent = XMFLOAT3(0.0f, 0.0f, 0.0f)) {
		m_xmf3Position = xmf3Position; m_xmf3Normal = xmf3Normal; m_xmf2TexCoord = xmf2TexCoord; m_xmf3TangentU = xmf3Tangent;
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
class CGridMeshIlluminated : public CMeshIlluminated
{
public:
	CGridMeshIlluminated(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float width, float depth, unsigned int m, unsigned int n);
	virtual ~CGridMeshIlluminated();
};
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
class CTreeVertex
...
private:
	XMFLOAT3 m_xmf3Position;
	XMFLOAT3 BillbordInfo;


public:

	CTreeVertex(XMFLOAT3 xmf3Position, XMFLOAT2 xmf2Size, float Type)
	{
		m_xmf3Position = xmf3Position;
		BillbordInfo.x = xmf2Size.x;
		BillbordInfo.y = xmf2Size.y;
		BillbordInfo.z = Type;
	}
	~CTreeVertex() {}
};
...
public:
	...
	CParticleVertex(XMFLOAT3 _xmf3Position, XMFLOAT3 _velocity, float _timer, UINT _type)
	{
		xmf3Position = _xmf3Position;
		velocity = _velocity;
		timer = _timer;
		type = _type;
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
class CVertex_GS : public CMesh
{
public:
	CVertex_GS(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, std::vector<CTreeVertex>& PointArray);
	virtual ~CVertex_GS();
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
class CParticle : public CMesh
{
public:
	ID3D12Resource* m_pd3dBufferFilledSize = NULL;
	...
	ID3D12Resource* m_pd3dStartBuffer = NULL;
	ID3D12Resource* m_pd3dStartUploadBuffer = NULL;

	ID3D12Resource* m_pd3dDrawBuffer = NULL;
	ID3D12Resource* m_pd3dDrawUploadBuffer = NULL;

	ID3D12Resource* m_pd3dStreamBuffer = NULL;
	ID3D12Resource* m_pd3dStreamUploadBuffer = NULL;

	D3D12_VERTEX_BUFFER_VIEW		m_d3dStartBufferView;
	D3D12_VERTEX_BUFFER_VIEW		m_d3dDrawBufferView;
	D3D12_STREAM_OUTPUT_BUFFER_VIEW		m_d3dStreamBufferView;

	int MAXPARTICLES = 128;
public:
	CParticle(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, D3D12_CPU_DESCRIPTOR_HANDLE uavStart);
	virtual ~CParticle();

	D3D12_VERTEX_BUFFER_VIEW GetStartBufferView() { return m_d3dStartBufferView; };
	D3D12_VERTEX_BUFFER_VIEW GetDrawBufferView() { return m_d3dDrawBufferView; };
	D3D12_STREAM_OUTPUT_BUFFER_VIEW GetStreamBufferView() { return m_d3dStreamBufferView; };
...
public:
	...
	float GetHeight(float x, float z,int cxQuadsPerBlock, int czQuadsPerBlock);
	XMFLOAT3 CubicBezierSum5x5(XMFLOAT3 patch[25], float u[5], float v[5]);
...
{
public:
	CEffectMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth = 20.0f, float fHeight = 20.0f);
	virtual ~CEffectMesh();
};

class CSkullMesh : public CMeshIlluminated
{
public:
