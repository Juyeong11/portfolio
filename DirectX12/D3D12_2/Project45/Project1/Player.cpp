...
void CPlayer::Update(float fTimeElapsed)
{
	...
	if (fLength) {
		curSpeed += fTimeElapsed;
	}
	else {
		curSpeed -= fTimeElapsed*10.f;
	}
	curSpeed = std::clamp(curSpeed, 0.f, 4.f);
...
CTerrainPlayer::CTerrainPlayer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext, int nMeshes) : CPlayer(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, pContext, nMeshes)
{
	...
	CCubeMeshIlluminated* pCubeMesh = new CCubeMeshIlluminated(pd3dDevice, pd3dCommandList, 4.0f, 12.0f, 4.0f);
	SetMesh(0, pCubeMesh);
	...
	pShader->CreateCbvSrvDescriptorHeaps(pd3dDevice, 1+nBullet, 0);
	...
	for (int i = 0; i < nBullet; ++i) {
		m_pBullets[i].BuildObject(pd3dDevice, pd3dCommandList);
		m_pBullets[i].SetCbvGPUDescriptorHandle(pShader->GetGPUCbvDescriptorNextHandle());
		pShader->CreateConstantBufferViews(pd3dDevice, 1, m_pBullets[i].m_pd3dcbGameObject, ncbElementBytes);

		
		m_pBullets[i].SetShader(pShader);
	}
...
void CTerrainPlayer::ReleaseUploadBuffers() {
	...
	for (int i = 0; i < 5; ++i)
		m_pBullets[i].ReleaseUploadBuffers();
}

void CTerrainPlayer::AnimateBullets(float fTimeElapsed)
{
	for (int i = 0; i < 5; ++i)
		m_pBullets[i].Animate(fTimeElapsed);
}
...
void CTerrainPlayer::FireBullet() 
{
	for (int i = 0; i < nBullet; ++i) {
		if (!m_pBullets[i].bActive) {
			m_pBullets[i].SetActive(true);
			m_pBullets[i].SetStartPosition(GetPosition());
			m_pBullets[i].SetShootDirection(m_xmf3Look, m_xmf3Right, m_xmf3Up);
			break;
		}
	}
...
CBoxPlayer::CBoxPlayer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext, int nMeshes) : CPlayer(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, pContext, nMeshes)
{
	m_pCamera = ChangeCamera(THIRD_PERSON_CAMERA, 0.0f);
	...
	SetPlayerUpdatedContext(pCubeMesh);
	SetCameraUpdatedContext(pCubeMesh);
}
...
void CBoxPlayer::ReleaseUploadBuffers() {
	CGameObject::ReleaseUploadBuffers();
...
void CBoxPlayer::AnimateBullets(float fTimeElapsed)
{
	for (int i = 0; i < 5; ++i)
		m_pBullets[i].Animate(fTimeElapsed);
}
...
void CBoxPlayer::OnPlayerUpdateCallback(float fTimeElapsed)
{

}
void CBoxPlayer::OnCameraUpdateCallback(float fTimeElapsed)
{
	XMFLOAT3 xmf3CameraPosition = m_pCamera->GetPosition();

}

void CBoxPlayer::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera) {
